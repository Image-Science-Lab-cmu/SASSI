...
def train_net(args, writer, dtype='train'):
    ...
    num_spixel = 25
    num_spixel_h = int(np.sqrt(num_spixel))
    num_spixel_w = int(np.sqrt(num_spixel))
    patch_size = [100, 100]
    dataloader = data.DataLoader(Dataset(num_spixel=num_spixel,
    ...
    model = create_ssn_net(num_spixels=num_spixel,
                           ...
                           num_spixels_h=num_spixel_h,
                           num_spixels_w=num_spixel_w,
                           dtype=dtype)
    # loss function
    criten = Loss()
            ...
            for epoch in range(100000):
                ...
                for iter, [inputs, num_h, num_w, init_index, cir_index, p2sp_index_, invisible] in enumerate(dataloader):
                    with torch.autograd.set_detect_anomaly(True):
                        ...
                        t1 = time.time()
                        recon_feat2, recon_label, centroids = model(img, p2sp_index_,
                                                                    invisible,
                                                                    init_index,
                                                                    cir_index,
                                                                    problabel,
